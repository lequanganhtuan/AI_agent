...
class InMemoryCache(BaseCache):
    ...
    async def set(self, key: str, report: FraudReport, ttl: int = 86400) -> None:
        async with self._lock:
            # Enforce max cache size to prevent OOM
            MAX_CACHE_SIZE = 200
            
            # 1. Clean up expired entries first to free up space
            current_time = time.monotonic()
            for k, (_, exp) in list(self._cache.items()):
                if current_time > exp:
                    del self._cache[k]
                    
            # 2. If still at capacity, evict the entry that expires the soonest (smallest absolute expiry)
            if len(self._cache) >= MAX_CACHE_SIZE and key not in self._cache:
                soonest_expire_key = min(self._cache.keys(), key=lambda k: self._cache[k][1])
                logger.info(f"Evicting cache key due to size limit ({MAX_CACHE_SIZE}): {soonest_expire_key}")
                del self._cache[soonest_expire_key]
