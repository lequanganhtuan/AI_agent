...
11. COGNITIVE TRUST MANIPULATION: Identify fake trust badges, fabricated security seals, counterfeit SSL indicators, fake customer testimonials, fake rolling popups of "recent commissions/earnings" or manufactured social proof elements.
...
14. CONTEXTUAL INCONSISTENCY (CRITICAL): Meticulously cross-reference the target URL's domain name with the actual visual content displayed on the page. If the domain suggests one purpose (e.g., "job", "recruitment", "support") but the page content displays completely unrelated elements (e.g., e-commerce products, random home appliances, "order-grabbing" tasks), highlight this mismatch as a high-risk indicator of a deceptive trap (such as "tasks fraud" or "fake order-grabbing" schemes).

CRITICAL ANALYSIS RULES:
- Evaluate ALL available evidence holistically. While you should avoid baseless speculation, you MUST NOT ignore obvious contextual inconsistencies. A target URL mimicking a brand on a non-official domain is SUFFICIENT evidence of Brand Impersonation, even if there is no login form present.
